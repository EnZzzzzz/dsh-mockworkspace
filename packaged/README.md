...
**会话快照（中途冻结）**：批次会话行的相机按钮「存快照」在**不打断对话**
的前提下冻结当前效果——Client `sessions.fork` 出快照会话（轨迹冻结副本，
改名「快照 · …」后归档隐藏、日志保留可恢复），Host `snapshot-session`
复制当前产物快照并写 `record.json`（`kind:'snapshot'` + `sourceSessionId` +
`note`，与归档共用同一条产物管线）。原会话继续对话；之后想从快照继续，
在 Hub 时间线该条目上点「继续对话」（fork 快照会话，快照本身不变）。
注意 fork 只冻结对话不冻结文件，所以产物必须快照当下复制；会话 running
时按钮禁用（产物是瞬态）。
